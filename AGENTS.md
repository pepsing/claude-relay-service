...
Codex Relay Service — 多平台 AI API 中转服务，作为客户端与上游 AI API 之间的中间件。
支持 Codex (官方/Console)、Gemini、OpenAI Responses、AWS Bedrock、Azure OpenAI、Droid、CCR 等账户类型。
...
- 用户要求提交并推送时，默认直接提交到 `main`，不要创建功能分支；仅在用户明确指定时使用其他分支
...
This project is indexed by GitNexus as **claude-relay-service** (12125 symbols, 26211 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/claude-relay-service/context` | Codebase overview, check index freshness |
| `gitnexus://repo/claude-relay-service/clusters` | All functional areas |
| `gitnexus://repo/claude-relay-service/processes` | All execution flows |
| `gitnexus://repo/claude-relay-service/process/{name}` | Step-by-step execution trace |
